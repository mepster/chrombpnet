...
def generate_bigwig(input_bam_file, input_fragment_file, input_tagalign_file, output_prefix, genome_fasta_file, chrom_sizes_file, plus_shift_delta, minus_shift_delta):
    ...
    # use unix sort (instead of "bedtools sort") because it uses temp files to avoid running out of RAM.
    # https://github.com/kundajelab/chrombpnet/issues/109
    ...
    # umm, shouldn't both sorts be "sort -k1,1 -k2n,2n" ??
    cmd = """awk -v OFS="\\t" '{{if ($6=="+"){{print $1,$2{0:+},$3,$4,$5,$6}} else if ($6=="-") {{print $1,$2,$3{1:+},$4,$5,$6}}}}' | sort -k1,1 -k2n,2n | bedtools genomecov -bg -5 -i stdin -g {2} | LC_COLLATE="C" sort -k1,1 -k2n,2n """.format(plus_shift_delta, minus_shift_delta, chrom_sizes_file)
